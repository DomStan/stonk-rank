...
def split_data_mixed(df, hold_out_time_slices, sample_proportion, seed=420):
    df_copy = df.copy()
    ...
    df_oos = df_copy[df_copy.trade_date.isin(dates_oos)].copy().sample(frac=1, random_state=seed)
    df_train = df_copy[df_copy.trade_date.isin(dates_train)].copy().sample(frac=1, random_state=seed)
    ...
    # No intersection
    assert len(
        set(df_oos.trade_date.unique()) & 
        set(df_train.trade_date.unique())
           ) == 0
    
    assert len(df) == sum([len(df_train), len(df_validation)])
    
    assert np.any(df_validation.index.isin(df_train.index)) == False
...
def transform_features(df, scalers=None, scaling=None, add_noise=False):
    ...
    # Select features
    df_copy = df_copy[['adf_pass_rate', 'last_residual', 'residual_mean_max', 'subindustry']].astype(data_types)
