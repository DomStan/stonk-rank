...
def performance_on_slice(df, y_score, y_preds, on_slice, top_ten):
    def _performance_summary(_df):
        ...
        if top_ten:
            ...
            performance_summary(top_scores['score'], top_scores['prediction'], top_scores['label'])
        else:
    ...
    df[[on_slice, 'prediction', 'score', 'label']].groupby(on_slice).apply(_performance_summary)
